...
export const emitNewFriendRequest = ({requestWithCreator, requestWithOther} : EmitNewFriendRequestProps) => {
	const io = getIO();
	const friendsNs = io.of('/friends')

	friendsNs.to(`user_${requestWithCreator.user_id_1}`).emit('newFriendRequest', {
...
export const emitUpdateFriendRequest = ({request, userId, isSender, action}: EmitHandleFriendRequest) => {
	const io = getIO();
	const friendsNs = io.of('/friends');

	const otherId = userId === request.user_id_1 ?
