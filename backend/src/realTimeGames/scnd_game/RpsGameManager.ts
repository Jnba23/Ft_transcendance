...
class RpsGameManager {
	...
	createGame(match: MmTypes.MatchResult): void {
		let rpsGame: RpsTypes.RpsGameState = {
			gameId: match.gameId,
			player1: {
	...
	startAutoChoiceTimer(gameId: string, userId: number, letEmKnow: () => void): void{
		const game = this.games.get(gameId);
		if (!game) return ;
	...
	startReconnectionTimer(gameId: string, userId: number, onTimeout: () => void): void {
		const game = this.games.get(gameId);
		if (!game) return;
		game.timers.reconnection = setTimeout(() => {
			const winner = game.player1.userId === userId ? game.player2.userId : game.player1.userId;
	...
	private determineWinner(choice1: RpsTypes.Choice, choice2: RpsTypes.Choice): number {
		if (choice1 === choice2)
			return 0;
