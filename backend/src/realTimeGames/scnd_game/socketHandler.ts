...
import * as RpsTypes from "../scnd_game/types.js"
...
export const setupRpsHandler = (io: Server) => {
	...
	RpsNs.on("connection", (socket) => {
		...
		socket.on("join-game", (data: { gameId: string, userId: number }) => {
			...
			if (matchInfo.player1.userId !== userId && matchInfo.player2.userId != userId){
			...
			if (!RpsServ.getGame(gameId))
				RpsServ.createGame(matchInfo);
			...
			if (RpsServ.bothPlayersReady(gameId)){
				...
				game.timers.roundReveal = setTimeout(() => {
					...
					if (game.phase === "game-over"){
						if (!game.winner){
							RpsNs.to(gameId).emit("error", { message: "Game can't end without a winner !"});
							return ;
						}
						saveCompleteGames({
							gameId: game.gameId,
						...
						RpsNs.to(gameId).emit('game-over', {
							winnerId: game.winner,
							winnerName: game.winner === game.player1.userId ? 
		...
		socket.on('disconnect', (reason) => {
			...
			RpsServ.startReconnectionTimer(gameId, userId, () => {
				RpsNs.to(gameId).emit('game-over', {
					winnerId: game.winner,
					winnerName: game.winner === game.player1.userId ? game.player1.name : game.player2.name,
					reason: "Forfeit",
