...
export const emitNewConversation = ({conversationId, creator, otherUser}: EmitNewConversationProps) => {
	const io = getIO();
...
export const emitNewMessage = ({senderId, receiverId, message}: EmitNewMessageProps) => {
	const io = getIO();
	const chatNs = io.of('/chat');	
	const payload = {
