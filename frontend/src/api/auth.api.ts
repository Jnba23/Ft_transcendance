...
export const authAPI = {
    ...
    login: async (data: LoginReq) => {
        ...
        const response = await client.post<LoginRes | Login2FARequiredRes>('/auth/login', data);
        return response.data;
    },

    logout: async () => {
        const response = await client.post<{ status: string; message: string }>('/auth/logout');
        return response.data;
    },
