...
function HistoryHeader({ setPage, setGameType, page, totalPages }: HistoryHeaderProps) {
			...
			<div className="flex flex-wrap justify-between items-center gap-4 mb-4">
				...
				<div className="flex items-center gap-4">
					...
					<div className="flex gap-2 items-center">
						...
						].join(' ')} onClick={() => setPage(prev => (prev - 1) > 1 ? prev - 1 : 1)}>
						...
						<button className={[
							...
							"hover:bg-white/10 hover:text-white transition-colors",
							"rounded-lg border border-white/10"
						].join(' ')} onClick={() => setPage(prev => (prev + 1) < totalPages ? prev + 1 : totalPages)}>
							<span className="text-sm font-medium pr-1">
								Next
							</span>
							<span className="material-symbols-outlined !text-base">
					...
					<TwoOptionsToggle
						...
						opt2="RPS History"
						section="profile"
						onOpt1Select={() => {setGameType('pong'); setPage(INITIAL_PAGE)}}
						onOpt2Select={() => {setGameType('RPS'); setPage(INITIAL_PAGE)}}
					/>
				</div>
			</div>
			<div className={[
