...
const EndMatch = () => {
  ...
  return (
    <div className="relative flex flex-col justify-center items-center min-h-screen w-full bg-background-dark p-4 md:p-8 gap-8 md:gap-12 text-center isolate overflow-x-hidden">
      <h1
        className={`font-bold text-5xl md:text-7xl leading-none ${didIWin ? 'text-blue-400' : 'text-red-400'}`}
      >
        {didIWin ? 'You Win' : 'You Loss'}
      </h1>

      <div
        className={`flex flex-col md:flex-row gap-8 md:gap-16 p-8 md:p-12 w-full max-w-180 bg-[#16213E]/60 rounded-3xl border border-white/10 items-center justify-between ${didIWin ? 'border-blue-500/20' : 'border-red-500/20'}`}
      >
        ...
        <div className="flex flex-col items-center justify-center shrink-0">
          <span className="text-xs md:text-sm font-light tracking-[0.3em] text-white/50 mb-2 uppercase">
            Final Score
          </span>
          <p className="font-black text-5xl md:text-7xl text-white">
            {myScore}
            <span className="text-blue-500 mx-2 md:mx-4">-</span>
            {opponentScore}
          </p>
        </div>
