...
import { handleNewConversation, handleNewMessage } from '@realtime/chat/chat.handlers';
import { createFriendsSocket } from '@services/friends/socket';
import { handleNewFriendRequest, handleUpdateFriendRequest } from '@realtime/friends/friends.handlers';
...
function useAppHydration() {
	...
	const initializeFriends = useFriendsStore(
		(state) => state.initialize
	);
	...
	useEffect(() => { // prevent not logged user socket connection
		if (!me) return;
